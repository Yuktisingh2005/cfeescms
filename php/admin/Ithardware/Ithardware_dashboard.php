...
$count_query = "SELECT status, COUNT(*) AS count FROM complaints WHERE type = 'Hardware' GROUP BY status";
...
$complaints = $conn->query("
  SELECT c.complaint_id, c.title, c.description, c.status, c.created_at,
         c.assigned_engineer_username, c.location,
  ...
  WHERE c.type = 'Hardware' AND c.status IN ('Pending', 'Active', 'Review Pending')
...
<!-- Sidebar -->
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        <ul>
          <li><a href="MyProfile_Ithardware.php"><i class="fa fa-user"></i> User Profile</a></li>
...
<!-- Table -->
  ...
  <div class="action-box" data-status="Review Pending">
    ...
    <h3>Review Pending: <?php echo $counts['Review Pending']; ?></h3>
      ...
      <h3 class="section-title">IT Hardware Complaints</h3>
